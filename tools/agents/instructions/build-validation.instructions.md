...
Use for Go module, local binary, dependency, SQLite/schema migration, API,
worker, artifact, test, and CI-equivalent workflow changes.
...
Run only the narrower command when the task is docs-only or the broader suite would not add signal. Record cloud/provider checks that cannot run locally.
